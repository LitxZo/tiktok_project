...
func RelationActionDao(userId, toUserId int) error {
	...
	if count == 0 {
		...
		var user model.User
		if err := global.DB.Table(user.GetTableName()).Where("id = ?", userId).UpdateColumn("follow_count", gorm.Expr("follow_count + ?", 1)).Error; err != nil {
			// 回滚事务
			tx.Rollback()
			return errors.New("关注用户失败")
		}

		if err := global.DB.Table(user.GetTableName()).Where("id = ?", toUserId).UpdateColumn("follower_count", gorm.Expr("follower_count + ?", 1)).Error; err != nil {
			// 回滚事务
			tx.Rollback()
			return errors.New("关注用户失败")
		}
...
func RelationUndoActionDao(userId, toUserId int) error {
	...
	tx := global.DB.Begin()
	var count int64
	result := global.DB.Table(followRecord.GetTableName()).Where("user_id = ? AND follow_id = ? AND deleted_at IS NULL", userId, toUserId).Count(&count).Delete(&followRecord)
	if result.Error != nil {
		tx.Rollback()
...
func RelationFollowListDao(userId int) ([]dto.User, error) {
	...
	for _, v := range ids {
		user := model.User{}
		err := global.DB.Table(user.GetTableName()).Where("id = ?", v).First(&user).Error
		if err != nil {
			return nil, err
		}

		userList = append(userList, bindUserDaoToDto(user, true))
	}
	// if len(userList) == 0 {
	// 	return nil, nil
	// }
...
func RelationFollowerListDao(userId int) ([]dto.User, error) {
	...
	for _, v := range ids {
		...
		if err2 := global.DB.Table(followRecord.GetTableName()).Where("user_id = ? AND follow_id = ? AND deleted_at IS NULL", userId, v).Find(&followRecord).Error; err2 != nil {
			isFollow = false
		}
		userList = append(userList, bindUserDaoToDto(user, isFollow))
	}
	if len(userList) == 0 {
		return nil, nil
	}
...
func RelationFriendListDao(userId int) ([]dto.User, error) {
	var ids []int64
	result := global.DB.Table(model.FollowRecord{}.GetTableName()).Select("follow_id").Where("user_id= ? AND deleted_at IS NULl", userId).Find(&ids)
	if result.Error != nil {
		return nil, result.Error
	}
